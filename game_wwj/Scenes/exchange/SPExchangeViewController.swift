//
//  SPExchangeViewController.swift
//  game_wwj
//
...
import UIKit
class SPExchangeViewController: SDPortraitViewController, ViewModelAttaching {
...
private extension SPExchangeViewController {
    func configView() {
        ...
        self.rootFlexContainer.flex.define { [unowned self] flex in
            ...
            flex.addItem().direction(.column).define { [unowned self] flex in
                flex.addItem(self.theBankView).marginLeft(30~).marginRight(30~).direction(.row).alignItems(.center).define { [unowned self] flex in
...
extension SPExchangeViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if AppDefine.needDiamond {
            if kind == UICollectionView.elementKindSectionHeader {
                let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "UICollectionElementKindSectionHeader", for: indexPath) as! SDExchangeGlodSectionHeaderView
                header.exchangeDelegate = self;
                self.exchangeGoldHeaderView = header;
                return header
            }
        }
...
extension SPExchangeViewController: SDExchangeGoldDelegate{
