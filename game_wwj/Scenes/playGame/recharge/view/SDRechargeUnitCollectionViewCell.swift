//
//  SDRechargeUnitCollectionViewCell.swift
//  mk_swift_wawaji
//
...
class SDRechargeUnitCollectionViewCell: UICollectionViewCell, SDCollectionItemType {
...
private extension SDRechargeUnitCollectionViewCell {
    func configView() {
        self.contentView.addSubview(self.rootFlexContainer);
        self.rootFlexContainer.flex.define { [unowned self] flex in
            flex.addItem(self.theBgImageView).position(.absolute).width(100%).height(434~);
            flex.addItem().direction(.column).alignItems(.center).define { [unowned self] flex in
                ...
                flex.addItem(self.theLineView).width(320~).height(2~).marginTop(8~);
                flex.addItem().width(292~).height(82~).marginTop(28~).direction(.row).alignItems(.center).justifyContent(.center).define { [unowned self] flex in
                    if AppDefine.needDiamond {
                        ...
                        flex.addItem().direction(.row).alignItems(.start).define { [unowned self] flex in
                            flex.addItem(self.theContentDesLabel).width(220~);
                            flex.addItem(self.theFreeDiamondLogoImageView).width(46~).height(46~);
                        }
                    }
                    
                }
                flex.addItem().width(100%).marginTop(130~).height(63~).direction(.row).alignItems(.center).justifyContent(.spaceBetween).define { [unowned self] flex in
